...
import akka.persistence.couchbase.scaladsl
...
object CouchbaseReadJournal {
   ...
   */
  final val Identifier = scaladsl.CouchbaseReadJournal.Identifier
}
 ...
 *
 */
final class CouchbaseReadJournal(scaladslReadJournal: scaladsl.CouchbaseReadJournal)
    extends ReadJournal
   ...
   */
  lazy val session: CompletionStage[CouchbaseSession] =
    scaladslReadJournal.session.map(_.asJava)(ExecutionContexts.sameThreadExecutionContext).toJava
   ...
   */
  override def currentPersistenceIds(): Source[String, NotUsed] = scaladslReadJournal.currentPersistenceIds().asJava
